...
def _stream_inflate(length_extra_bits_diffs, dist_extra_bits_diffs, cache_size, chunk_size):
    ...
    def yield_exactly(bytes_to_yield):
        return DeferredYielder(can_proceed=get_true, to_yield=lambda: (bytes_to_yield,), num_from_cache=(0, 0), return_value=lambda: None)

    def yield_from_cache(dist, length):
        return DeferredYielder(can_proceed=get_true, to_yield=lambda: (), num_from_cache=(dist, length), return_value=lambda: None)
