...
def _stream_inflate(length_extra_bits_diffs, dist_extra_bits_diffs, cache_size, chunk_size):
    ...
    def get_deferred_yielder_readers(reader_has_bit, reader_has_byte, reader_get_bit, reader_get_byte, reader_yield_bytes_up_to):
        ...
        get_bit = DeferredYielder(can_proceed=reader_has_bit, to_yield=lambda: (), cache_dist=0, cache_length=0, return_value=reader_get_bit)
        get_byte = DeferredYielder(can_proceed=reader_has_byte, to_yield=lambda: (), cache_dist=0, cache_length=0, return_value=reader_get_byte)
        ...
        def yield_bytes(num_bytes):
            ...
            yield_bytes_up_to = DeferredYielder(can_proceed=reader_has_byte, to_yield=to_yield, cache_dist=0, cache_length=0, return_value=lambda: None)
    ...
    def get_runner(append, via_cache, from_cache, alg):
        is_done = False
        can_proceed, to_yield, cache_dist, cache_length, return_value = None, None, None, None, None

        def _is_done():
            return is_done

        def _run(new_iterable):
            nonlocal is_done, can_proceed, to_yield, cache_dist, cache_length, return_value
            ...
            while True:
                if can_proceed is None:
                    try:
                        can_proceed, to_yield, cache_dist, cache_length, return_value = \
                ...
                if cache_length:
                    yield from via_cache(from_cache(cache_dist, cache_length))
                can_proceed = None
    ...
    def yield_exactly(bytes_to_yield):
        return DeferredYielder(can_proceed=lambda: True, to_yield=lambda: (bytes_to_yield,), cache_dist=0, cache_length=0, return_value=lambda: None)

    def yield_from_cache(dist, length):
        return DeferredYielder(can_proceed=lambda: True, to_yield=lambda: (), cache_dist=dist, cache_length=length, return_value=lambda: None)
